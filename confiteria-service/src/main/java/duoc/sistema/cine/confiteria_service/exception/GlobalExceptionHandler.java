package duoc.sistema.cine.confiteria_service.exception;

import duoc.sistema.cine.confiteria_service.dto.ErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    // Algun error de los WebClient
    @ExceptionHandler(RecursoWebClientInvalido.class)
    public ResponseEntity<ErrorResponse> erroesWebClient(RecursoWebClientInvalido ex, HttpServletRequest request){
        log.warn("Error en los datos enviado por el WebClient {}", request.getRequestURI());
        ErrorResponse respuesta = new ErrorResponse();
        respuesta.setFecha(LocalDateTime.now());
        respuesta.setEstado(HttpStatus.BAD_REQUEST.value());
